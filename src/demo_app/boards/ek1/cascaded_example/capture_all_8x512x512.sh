...
# CTRX_C is connected to CSI-2 4 and 6, which can't be mapped to the same video device
# therefore the data has to be collected from 2 devices with half size each (CSI2 mode C)
VIDEO2_WIDTH=$((DEFAULT_WIDTH / 2))  # Half width
VIDEO3_WIDTH=$((DEFAULT_WIDTH / 2))  # Half width
...
run_v4l2_capture /dev/video3 $VIDEO3_WIDTH $OUTPUT_FOLDER/ctrx3_bin.raw &
PID3=$!
